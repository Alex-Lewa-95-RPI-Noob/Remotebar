...
public class RemotebarController extends UIController {
    ...
    private int currentRemotebarPosition = 0;
    ...
    private List<String> currentRemotebarIds = new ArrayList<>();
    ...
    public RemotebarController(Context context) {
        super(context);
    }

    @Override
    public void onCreateHost(UIHost host) {
        init(context, host, () -> drawRemotebar(host));
    }

    private void drawRemotebar(UIHost host) {
        IconCache.getInstance(context).clearCache();

        // Initialize layout params
        WindowManager windowManager = (WindowManager) context.getSystemService(Context.WINDOW_SERVICE);
        RemotebarPosition.setCachedRotation(windowManager.getDefaultDisplay().getRotation());
        ...
        positionIsVertical = RemotebarPosition.isVertical(taskbarPosition);
        ...
        space.setOnClickListener(v -> toggleRemotebar(true));
        ...
        updateParamsRunnable = () -> {
            ...
            if(RemotebarPosition.isVertical(context)) {
    ...
    @VisibleForTesting
    int getRemotebarGravity(String taskbarPosition) {
                    ...
                    if(showHideAutomagically && !positionIsVertical && !MenuHelper.getInstance().isStartMenuOpen()) {
                        handler.post(() -> {
                            if(layout != null) {
                                ...
                                if(location[1] != 0) {
                                    if(location[1] > currentRemotebarPosition) {
                                        currentRemotebarPosition = location[1];
                                    } else if(location[1] < currentRemotebarPosition) {
                                        if(currentRemotebarPosition - location[1] == getNavBarSize())
                                            currentRemotebarPosition = location[1];
                                        else if(!startThread2) {
                                            startThread2 = true;
                                            tempHideRemotebar(true);
                                        }
                                    }
                                }
                            }
    ...
    private void updateRecentApps(final boolean firstRefresh) {
        ...
        if(usageStatsList.size() > 0 || realNumOfPinnedApps > 0 || fullLength) {
            if(realNumOfPinnedApps < maxNumOfEntries) {
                ...
                // Also filter out the current launcher, and Remotebar itself
            ...
            // we need to determine if we need to redraw the Remotebar or not
            boolean shouldRedrawRemotebar = firstRefresh;
            ...
            if(finalApplicationIds.size() != currentRemotebarIds.size()
                    || numOfPinnedApps != realNumOfPinnedApps
                    || numOfSysTrayIcons != realNumOfSysTrayIcons)
                shouldRedrawRemotebar = true;
            else {
                for(int i = 0; i < finalApplicationIds.size(); i++) {
                    if(!finalApplicationIds.get(i).equals(currentRemotebarIds.get(i))) {
                        shouldRedrawRemotebar = true;
                        break;
                    }
                }
            ...
            if(shouldRedrawRemotebar) {
                currentRemotebarIds = finalApplicationIds;
                numOfPinnedApps = realNumOfPinnedApps;
                numOfSysTrayIcons = realNumOfSysTrayIcons;
        ...
        } else if(firstRefresh || currentRemotebarIds.size() > 0) {
            currentRemotebarIds.clear();
            handler.post(() -> {
                isShowingRecents = false;
                scrollView.setVisibility(View.GONE);
    ...
    void calculateScrollViewParams(Context context,
        ...
        float baseStart = U.getBaseRemotebarSizeStart(context);
        float baseEnd = U.getBaseRemotebarSizeEnd(context, sysTrayIconStates);
        ...
        boolean realMatchParent = maxRecentsSize >= maxScreenSize
                ...
                && !(RemotebarPosition.isVertical(context) && U.isChromeOs(context));
    ...
    boolean needToReverseOrder(Context context, String sortOrder) {
        switch(RemotebarPosition.getRemotebarPosition(context)) {
            case POSITION_BOTTOM_RIGHT:
            case POSITION_TOP_RIGHT:
    ...
    private void toggleRemotebar(boolean userInitiated) {
    ...
    private void showRemotebar(boolean clearVariables) {
        if(clearVariables) {
    ...
    private void hideRemotebar(boolean clearVariables) {
        if(clearVariables) {
            taskbarShownTemporarily = false;
            taskbarHiddenTemporarily = false;
        }
    ...
    private void tempShowRemotebar() {
    ...
    private void tempHideRemotebar(boolean monitorPositionChanges) {
    ...
    private boolean checkPositionChange() {
        if(!isScreenOff() && layout != null) {
            ...
            if(location[1] == 0) {
                ...
                } else if(location[1] == currentRemotebarPosition && taskbarHiddenTemporarily) {
                    tempShowRemotebar();
                    return true;
                } else if(location[1] < currentRemotebarPosition
                        && currentRemotebarPosition - location[1] == getNavBarSize()) {
                    currentRemotebarPosition = location[1];
                }
            }
        }
