...
import com.openlewa.remotebar.Constants
import com.openlewa.remotebar.LauncherAppsHelper.generateTestLauncherActivityInfo
...
import com.openlewa.remotebar.mockito.StringAnswer
import com.openlewa.remotebar.shadow.RemotebarShadowScrollView
...
import com.openlewa.remotebar.util.Constants.PREF_BUTTON_BACK
import com.openlewa.remotebar.util.Constants.PREF_BUTTON_HOME
import com.openlewa.remotebar.util.Constants.PREF_BUTTON_RECENTS
import com.openlewa.remotebar.util.Constants.PREF_DASHBOARD
import com.openlewa.remotebar.util.Constants.PREF_HIDE_FOREGROUND
import com.openlewa.remotebar.util.Constants.PREF_RECENTS_AMOUNT
import com.openlewa.remotebar.util.Constants.PREF_RECENTS_AMOUNT_APP_START
import com.openlewa.remotebar.util.Constants.PREF_RECENTS_AMOUNT_RUNNING_APPS_ONLY
import com.openlewa.remotebar.util.Constants.PREF_RECENTS_AMOUNT_SHOW_ALL
import com.openlewa.remotebar.util.Constants.PREF_START_BUTTON_IMAGE
import com.openlewa.remotebar.util.Constants.PREF_START_BUTTON_IMAGE_APP_LOGO
import com.openlewa.remotebar.util.Constants.PREF_START_BUTTON_IMAGE_CUSTOM
import com.openlewa.remotebar.util.Constants.PREF_START_BUTTON_IMAGE_DEFAULT
import com.openlewa.remotebar.util.Constants.PREF_TIME_OF_SERVICE_START
import com.openlewa.remotebar.util.RemotebarPosition
...
@PrepareForTest(value = [U::class, RemotebarPosition::class])
class RemotebarControllerTest {
    @get:Rule
    val rule = PowerMockRule()
    private lateinit var uiController: RemotebarController
    ...
    fun setUp() {
        ...
        uiController = RemotebarController(context)
    ...
    @Test
    fun testGetRemotebarGravity() {
        Assert.assertEquals(
                (Gravity.BOTTOM or Gravity.LEFT).toLong(),
                uiController.getRemotebarGravity(POSITION_BOTTOM_LEFT).toLong())
        Assert.assertEquals(
                (Gravity.BOTTOM or Gravity.LEFT).toLong(),
                uiController.getRemotebarGravity(POSITION_BOTTOM_VERTICAL_LEFT).toLong())
        Assert.assertEquals(
                (Gravity.BOTTOM or Gravity.RIGHT).toLong(),
                uiController.getRemotebarGravity(POSITION_BOTTOM_RIGHT).toLong())
        Assert.assertEquals(
                (Gravity.BOTTOM or Gravity.RIGHT).toLong(),
                uiController.getRemotebarGravity(POSITION_BOTTOM_VERTICAL_RIGHT).toLong())
        Assert.assertEquals(
                (Gravity.TOP or Gravity.LEFT).toLong(),
                uiController.getRemotebarGravity(POSITION_TOP_LEFT).toLong())
        Assert.assertEquals(
                (Gravity.TOP or Gravity.LEFT).toLong(),
                uiController.getRemotebarGravity(POSITION_TOP_VERTICAL_LEFT).toLong())
        Assert.assertEquals(
                (Gravity.TOP or Gravity.RIGHT).toLong(),
                uiController.getRemotebarGravity(POSITION_TOP_RIGHT).toLong())
        Assert.assertEquals(
                (Gravity.TOP or Gravity.RIGHT).toLong(),
                uiController.getRemotebarGravity(POSITION_TOP_VERTICAL_RIGHT).toLong())
        Assert.assertEquals(
                (Gravity.BOTTOM or Gravity.LEFT).toLong(),
                uiController.getRemotebarGravity(Constants.UNSUPPORTED).toLong())
    }
    ...
    fun testCalculateScrollViewParams() {
        ...
        PowerMockito.`when`(RemotebarPosition.isVertical(context)).thenAnswer(isVerticalAnswer)
        ...
        var maxScreenSize = Math.max(
                ...
                display.height -
                        ...
                        Math.round(U.getBaseRemotebarSize(context))
        )
        ...
        maxScreenSize = Math.max(0, display.width - Math.round(U.getBaseRemotebarSize(context)))
        uiController.calculateScrollViewParams(context, prefs, params, true, 1)
    ...
    @Test
    fun testScrollRemotebarForScrollViewVisibility() {
        val layoutId = uiController.getRemotebarLayoutId(POSITION_BOTTOM_LEFT)
        val layout = LayoutInflater.from(context).inflate(layoutId, null) as LinearLayout
    ...
    @Test
    @Config(shadows = [RemotebarShadowScrollView::class])
    fun testScrollRemotebarForScrollViewLocation() {
    ...
    fun testFilterForegroundApp() {
        ...
        uiController.filterForegroundApp(context, prefs, searchInterval, applicationIdsToRemove)
        Assert.assertEquals(entryTestPackage1, applicationIdsToRemove.removeAt(0))
        event = buildRemotebarForegroundAppEvent(MainActivity::class.java.canonicalName, 300L)
        Shadows.shadowOf(usageStatsManager).addEvent(event)
        uiController.filterForegroundApp(context, prefs, searchInterval, applicationIdsToRemove)
        Assert.assertEquals(MainActivity::class.java.canonicalName,
                applicationIdsToRemove.removeAt(0))
        event = buildRemotebarForegroundAppEvent(HomeActivity::class.java.canonicalName, 400L)
        Shadows.shadowOf(usageStatsManager).addEvent(event)
        uiController.filterForegroundApp(context, prefs, searchInterval, applicationIdsToRemove)
        Assert.assertEquals(HomeActivity::class.java.canonicalName,
                applicationIdsToRemove.removeAt(0))
        event = buildRemotebarForegroundAppEvent(
        ...
        Assert.assertEquals(InvisibleActivityFreeform::class.java.canonicalName,
                applicationIdsToRemove.removeAt(0))
        event = buildRemotebarForegroundAppEvent(Constants.UNSUPPORTED, 800L)
        Shadows.shadowOf(usageStatsManager).addEvent(event)
        uiController.filterForegroundApp(context, prefs, searchInterval, applicationIdsToRemove)
    ...
    fun testNeedToReverseOrder() {
        ...
        for (position in positions) {
            PowerMockito.`when`(RemotebarPosition.getRemotebarPosition(context))
    ...
    private fun buildRemotebarForegroundAppEvent(className: String, timestamp: Long):
            UsageEvents.Event {
    ...
    private fun initializeSysTrayLayout(position: String): LinearLayout {
        val layoutId = uiController.getRemotebarLayoutId(position)
        val layout = LayoutInflater.from(context).inflate(layoutId, null) as LinearLayout
        uiController.drawSysTray(context, layoutId, layout)
        return getFieldSysTrayLayout(uiController)
    }

    private fun getFieldSysTrayLayout(uiController: RemotebarController?): LinearLayout {
